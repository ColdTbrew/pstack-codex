...
Spawn all explorers concurrently with the read-only `pstack_explorer` custom agent. If the requested fan-out exceeds the available Codex slots, use waves and keep each angle distinct.
...
Spawn a single read-only `pstack_reviewer_sol` agent that explores and explains in one pass.
...
Once all explorers return, spawn `pstack_reviewer_sol` to synthesize their findings into one coherent explanation.
...
After the explanation is complete, launch `pstack_reviewer_sol`, `pstack_reviewer_terra`, and `pstack_reviewer_luna` concurrently as architectural critics.
