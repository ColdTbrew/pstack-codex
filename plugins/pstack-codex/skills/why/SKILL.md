...
Before spawning investigators, inspect the available Codex tools, plugins, and MCPs. Use the available-tools map or plugin/tool discovery rather than assuming a connector exists.
...
Use the `pstack_explorer` custom agent for each category. It is read-only and inherits the parent's available tools and connectors. If a required connector is not available to subagents, run that category in the parent instead of weakening the evidence coverage.
...
Spawn `pstack_reviewer_sol` as the read-only synthesizer. It inherits the parent's available tools; if it cannot reach a cited source, the parent spot-verifies that citation before presenting the result.
