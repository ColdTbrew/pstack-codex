...
Create or update a Codex plan with the steps below copied in verbatim. A step you skip stays listed with `skip: <reason>`.
...
Create `.audit/orchestrate/<project-slug>/` in the current workspace. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Use `bun scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe ask the user state.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Subagents may reference the standing-orders file by workspace path; every replacement spawn receives the full consolidated scope.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget within the session limit, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Run restacks in an isolated worktree and serialize them with other stack mutations.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts. Use a `pstack_reviewer_*` profile that did not write the unit when verification is expensive, judgment-laden, or high-blast-radius. A verifier whose entire product would be rerunning one command is ceremony, not verification.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the Codex subagent's status in the Codex dashboard. Transcript mtime is not liveness.
...
- Retry by mode: cap-hit or OOM, respawn with smaller scope; network-drop, retry as-is; tool-error, retry once with another suitable `pstack_*` profile; unknown, retry once. Two retries, then abandon the unit and replan around it.
...
- After a Codex restart: re-read the standing orders and `units.tsv`, recompute the frontier, inspect surviving tasks, PRs, branches, and worktrees, then respawn only units without a live owner. Reattach by PR and branch rather than transient agent id.
