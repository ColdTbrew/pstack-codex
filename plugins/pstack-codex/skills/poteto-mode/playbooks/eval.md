...
4. **Spawn N parallel candidates** through the **arena** skill's Phase B. Each profile works in its own sanitized directory with the same prompt.
5. **Spawn one blinded judge** through the **arena** skill's Phase C. The judge sees outputs by sanitized label and the rubric, never a profile name.
6. **Verify the chain from evidence, not self-report.** Inspect each candidate's tool trace when available, files changed or read, and produced artifact. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from observable work plus the code shape, never from the candidate's claims alone.
