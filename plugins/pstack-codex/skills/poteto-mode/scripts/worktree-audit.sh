...
# Codex stores task transcripts as rollout JSONL. Search active and archived
# sessions because either can be the most recent task that touched a worktree.
codex_home="${CODEX_HOME:-$HOME/.codex}"
transcript_roots=()
[ -d "$codex_home/sessions" ] && transcript_roots+=("$codex_home/sessions")
[ -d "$codex_home/archived_sessions" ] && transcript_roots+=("$codex_home/archived_sessions")
now=$(date +%s)
...
git worktree list --porcelain | awk '/^worktree /{print $2}' | while read -r wt; do
	...
	if [ "${#transcript_roots[@]}" -gt 0 ]; then
		f=$(rg -l -e "${wt}/" -e "${wt}\"" "${transcript_roots[@]}" 2>/dev/null \
			| while IFS= read -r path; do
				stat -c '%Y %n' "$path" 2>/dev/null \
					|| stat -f '%m %N' "$path" 2>/dev/null
			done | sort -rn | head -1)
		if [ -n "$f" ]; then
			last_ts=$(printf '%s\n' "$f" | awk '{print $1}')
