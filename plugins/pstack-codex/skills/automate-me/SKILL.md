...
Use the current task first. When the user asks for broader history, use Codex task/thread tools and the user's local Codex memory index when available. Select only tasks relevant to the named project or working style; do not broadly mine unrelated conversations.

Survey the selected tasks for recurring patterns. Split a large history into up to three read-only `pstack_explorer` slices. Each slice returns a short structured list of patterns with task or evidence pointers. Default signals worth hunting:
...
- Add `agents/openai.yaml` with `policy.allow_implicit_invocation: false` by default. Mode skills are heavy and opinionated, so they should apply only when explicitly invoked. Keep implicit invocation only when the user explicitly wants the mode applied automatically.
